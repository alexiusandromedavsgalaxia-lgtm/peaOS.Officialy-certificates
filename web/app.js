...
function render() {
  ...
  app.innerHTML = `
    ...
    <section class="section">
      <div class="section-head"><div><div class="eyebrow">PROTOCOLO</div><h2>R35-SHA512</h2><div class="muted">registro verificable y separado de WDP.</div></div></div>
      <div class="stats">
      ...
      <div class="card wdp">
        ...
        <div class="actions"><button class="secondary" data-action="sync">Sincronizar ahora</button><a class="secondary" href="${CERT_REGISTRY}" target="_blank" rel="noopener noreferrer">Abrir registry.json ↗</a></div>
        ${integration.error ? `<div class="notice error-notice"><span>${esc(integration.error)}</span><button type="button" class="secondary" data-action="sync">Reintentar</button></div>` : ''}
      </div>
    </section>

    <section id="certificates" class="section">
      <div class="section-head"><div><div class="eyebrow">REGISTRY</div><h2>certificados de firma</h2><div class="muted">oficiales remotos + tus borradores locales.</div></div><button class="primary" data-action="new-cert">+ Nuevo certificado</button></div>
      <div class="stats compact"><div class="card stat"><strong>${official}</strong><span>oficiales</span></div><div class="card stat"><strong>${drafts}</strong><span>borradores</span></div><div class="card stat"><strong>${active}</strong><span>activos locales</span></div></div>
      <div class="cert-grid">${integration.registry?.certificates?.map(renderOfficialCertificate).join('') || ''}${state.certificates.map(renderLocalCertificate).join('')}${!official && !drafts ? `<div class="card empty"><div class="empty-icon">◇</div><h3>registro vacío</h3><p>No hay certificados publicados todavía.</p><button type="button" class="secondary" data-action="new-cert">Crear un borrador local</button></div>` : ''}</div>
    </section>

    <section id="wdp" class="section"><div class="card wdp"><div class="eyebrow">WEB DISTRIBUTION PROGRAM</div><h2>WDP va por separado.</h2><p class="muted">Las solicitudes de distribución web no se mezclan con el certificado criptográfico. Aquí solo se guardan borradores locales.</p><button type="button" class="primary" data-action="wdp">${state.wdp ? 'Ver borrador WDP' : 'Crear borrador WDP'} <span>→</span></button>${state.wdp ? `<div class="notice"><span>${esc(state.wdp.origin)}</span><b>${esc(state.wdp.id)}</b></div>` : ''}</div></section>
...
function renderOfficialCertificate(certificate) {
  ...
  return `<article class="card cert"><div class="cert-top"><span class="badge ${status.toLowerCase()}">${esc(status)}</span><span class="cert-type">SigningCertificate</span></div><h3>${esc(certificate.id)}</h3><div class="cert-meta"><div><span>issuer key</span><b>${esc(certificate.issuer_key_id || '—')}</b></div><div><span>serial</span><b>${esc(certificate.serial || '—')}</b></div><div><span>SHA-512</span><code>${esc(certificate.sha512 || '—')}</code></div><div><span>vigencia</span><b>${esc(certificate.issued_at || '—')} → ${esc(certificate.expires_at || '—')}</b></div></div><div class="source">● GitHub R35 · oficial</div></article>`;
}
...
function updateAccountButton() {
  ...
  accountButton.innerHTML = state.user ? `<span class="signed"><span class="avatar">${esc(initials(state.user.name))}</span>${esc(state.user.name)}</span>` : 'Iniciar sesión';
}
...
function createLocalCertificate() {
  if (!state.user) { certDialog.close(); openAuth('signup'); return; }
  const name = String(new FormData(certForm).get('name') || '').trim();
  if (!name) { certFormError.textContent = 'Escribe un nombre.'; return; }
  state.certificates.push({ id: makeId('LOCAL-CERT'), name, status: 'Active', protocol: 'R35-SHA512', algorithm: 'Ed25519', createdAt: new Date().toISOString() });
  saveState(); certForm.reset(); certFormError.textContent = ''; certDialog.close(); render();
}
...
function saveWdp() {
  ...
  try {
    ...
    saveState(); authDialog.close(); render();
  } catch { error.textContent = 'Usa una URL HTTPS válida.'; }
}

function deleteWdp() {
  state.wdp = null; saveState(); authDialog.close(); render();
}
...
app.addEventListener('click', event => {
  ...
  else if (action === 'new-cert') state.user ? certDialog.showModal() : openAuth('signup');
  else if (action === 'sync') syncPeaOS();
  else if (action === 'wdp') state.user ? showWdp() : openAuth('signup');
  else if (action === 'delete-cert') deleteCertificate(actionElement.dataset.id);
});
